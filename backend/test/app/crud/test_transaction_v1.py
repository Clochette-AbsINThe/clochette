...
class TestCRUDTransactionV1(BaseTest):
    ...
    async def test_create_transaction_sale_false(self):
        with patch("app.crud.crud_transaction_v1.treasuries.add_transaction") as mock_add_transaction:
            ...
            # Act
            async with get_db.get_session() as session:
                transaction_in_db = await crud_transaction_v1.create(session, obj_in=transaction_create)

                consumable_in_db = await crud_consumable.query(session)
                out_of_stock_in_db = await crud_out_of_stock.query(session)

            # Assert
            assert consumable_in_db[0].transaction_v1_id_purchase == transaction_in_db.id
    ...
    async def test_create_transaction_sale_true(self):
        with patch("app.crud.crud_transaction_v1.treasuries.add_transaction") as mock_add_transaction:
            ...
            # Act
            async with get_db.get_session() as session:
                transaction_in_db_1 = await crud_transaction_v1.create(session, obj_in=transaction_create_consumable)
                ...
                transaction_in_db_2 = await crud_transaction_v1.create(session, obj_in=transaction_create)

                consumable_in_db = await crud_consumable.query(session)
                out_of_stock_in_db = await crud_out_of_stock.query(session)

            # Assert
            assert consumable_in_db[0].transaction_v1_id_purchase == transaction_in_db_1.id
