...
async def read_out_of_stock_item(out_of_stock_item_id: int, db: DBDependency):
    """
    ...
    if out_of_stock_item is None:
        logger.debug("Out of stock item %s not found", out_of_stock_item_id)
        raise HTTPException(
...
async def create_out_of_stock_item(
    ...
    if await out_of_stock_items.query(
        ...
        logger.debug("Out of stock item %s already exists", out_of_stock_item.name)
        raise HTTPException(
...
async def update_out_of_stock_item(
    ...
    return await out_of_stock_items.update(
        db,
        db_obj=old_out_of_stock_item,
        obj_in=out_of_stock_item,
    )
...
async def delete_out_of_stock_item(out_of_stock_item_id: int, db: DBDependency):
    """
    ...
    if await out_of_stocks.query(db, limit=1, item_id=out_of_stock_item_id):
        logger.debug("Out of stock item %s is used", out_of_stock_item_id)
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
