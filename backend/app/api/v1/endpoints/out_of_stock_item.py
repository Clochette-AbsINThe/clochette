...
@router.get("/buy/", response_model=list[out_of_stock_item_schema.OutOfStockItem], response_model_exclude_none=True, dependencies=[Security(get_current_active_account)])
async def read_out_of_stock_items_buy(db=Depends(get_db), query=Depends(to_query_parameters(out_of_stock_item_schema.OutOfStockItemBase))) -> list:
    return await out_of_stock_items.query(db, limit=None, buy_or_sell=True, **process_query_parameters(query))


@router.get("/sell/", response_model=list[out_of_stock_item_schema.OutOfStockItem], response_model_exclude_none=True, dependencies=[Security(get_current_active_account)])
async def read_out_of_stock_items_sell(db=Depends(get_db), query=Depends(to_query_parameters(out_of_stock_item_schema.OutOfStockItemBase))) -> list:
    return await out_of_stock_items.query(db, limit=None, buy_or_sell=False, **process_query_parameters(query))


@router.get("/{out_of_stock_item_id}", response_model=out_of_stock_item_schema.OutOfStockItem, response_model_exclude_none=True, dependencies=[Security(get_current_active_account)])
...
@router.post("/", response_model=out_of_stock_item_schema.OutOfStockItem, response_model_exclude_none=True, dependencies=[Security(get_current_active_account)])
async def create_out_of_stock_item(out_of_stock_item: out_of_stock_item_schema.OutOfStockItemCreateFront, db=Depends(get_db)) -> dict:
    if await out_of_stock_items.query(db, limit=1, name=out_of_stock_item.name, buy_or_sell=out_of_stock_item.sell_price is None):
        raise HTTPException(
    ...
    return await out_of_stock_items.create(db, obj_in=saved_model.dict(by_alias=False))


@router.put("/{out_of_stock_item_id}", response_model=out_of_stock_item_schema.OutOfStockItem, response_model_exclude_none=True, dependencies=[Security(get_current_active_account)])
async def update_out_of_stock_item(out_of_stock_item_id: int, out_of_stock_item: out_of_stock_item_schema.OutOfStockItemUpdateFront, db=Depends(get_db)):
    ...
    results = await out_of_stock_items.query(db, limit=1, name=out_of_stock_item.name, buy_or_sell=out_of_stock_item.sell_price is None)
    ...
    return await out_of_stock_items.update(db, db_obj=old_out_of_stock_item, obj_in=saved_model)


@router.delete("/{out_of_stock_item_id}", response_model=out_of_stock_item_schema.OutOfStockItem, response_model_exclude_none=True, dependencies=[Security(get_current_active_account)])
