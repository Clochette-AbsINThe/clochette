...
@router.get("/", response_model=list[consumable_item_schema.ConsumableItem], dependencies=[Security(get_current_active_account)])
async def read_consumable_items(db=Depends(get_db)) -> list:
    return await consumable_items.query(db, limit=None)


@router.post("/", response_model=consumable_item_schema.ConsumableItem, dependencies=[Security(get_current_active_account)])
...
@router.get("/{consumable_item_id}", response_model=consumable_item_schema.ConsumableItem, dependencies=[Security(get_current_active_account)])
...
@router.put("/{consumable_item_id}", response_model=consumable_item_schema.ConsumableItem, dependencies=[Security(get_current_active_account)])
async def update_consumable_item(consumable_item_id: int, consumable_item: consumable_item_schema.ConsumableItemUpdate, db=Depends(get_db)):
    ...
    results = await consumable_items.query(db, name=consumable_item.name, limit=None)
    if results and results[0].id != old_consumable_item.id:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=translator.ELEMENT_ALREADY_EXISTS
        )
    return await consumable_items.update(db, db_obj=old_consumable_item, obj_in=consumable_item)


@router.delete("/{consumable_item_id}", response_model=consumable_item_schema.ConsumableItem, dependencies=[Security(get_current_active_account)])
