...
@handle_exceptions(
    consumable_translator.DELETION_OF_USED_ELEMENT,
    InvalidTransaction,
    sync_func=True,
)
def prevent_consumables_deletion(target: Transaction):
    ...
    if any(consumable.transaction_id_sale for consumable in target.consumables_purchase):
        logger.debug("Preventing transaction deletion for consumable")
        msg = "Cannot delete transaction with associated consumable sales"
...
@handle_exceptions(
    barrel_translator.DELETION_OF_USED_ELEMENT,
    InvalidTransaction,
    sync_func=True,
)
def prevent_barrels_deletion(target: Transaction):
    ...
    if any(barrel.transaction_id_sale for barrel in target.barrels_purchase):
        ...
        msg = "Cannot delete transaction with associated barrel sales"
        raise InvalidTransaction(
            msg,
        )
