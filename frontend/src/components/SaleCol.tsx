...
export default function SaleCol(props: SaleColProps): JSX.Element {
    ...
    return (
        <div className="h-full flex flex-col border rounded border-gray-800 dark:border-gray-300">
            ...
            {items.map((item, index) => (item.item.name === 'EcoCup')
                ? (
                    <div className='flex flex-col grow justify-end' key={index} aria-label='glass-div'>
                        <ItemCount item={item} onValueChange={onValueChange} />
                    </div>
                )
                : (
                    <ItemCount item={item} onValueChange={onValueChange} key={index} />
                )
            )}
        </div>
    );
