...
export default function PopupWindows(props: PopupWindowsProps): JSX.Element {
        ...
        if (isOpen) {
            return (
                <div className="fixed top-0 left-0 w-full h-full flex items-center justify-center bg-[#88888888] px-3">
                    <div ref={ref} className="bg-gray-100 dark:bg-slate-900 rounded-lg p-4 flex flex-col border-2 dark:border-white border-black min-w-[75vw] min-h-[75vh] opacity-100" key="popup" id="PopUp" aria-label='PopUp' role='dialog'>
                        <button className="ml-auto flex justify-end max-w-min" onClick={close} aria-label='closeButton'>
    ...
    return (
        <>
            {props.trigger && <TriggerButton className={props.trigger.className} content={props.trigger.content} onClick={open} />}
            {renderContent()}
        </>
    );
};
...
function TriggerButton(props: TriggerButtonProps): JSX.Element {
    return (<button className={props.className} onClick={props.onClick} aria-label='button-popup'>{props.content}</button>);
}
