import { useEffect, useState } from 'react';

export interface ItemCountProps {
    id?: number
    name: string
    price: number
    onValueChange?: (value: number, key: number) => void
    value?: number
    image?: string
}

export function ItemCount(props: ItemCountProps): JSX.Element {
    const [count, setCount] = useState(0);

    const resetHandler = (): void => {
        setCount(-(props.value ?? 0));
    };

    const incrementMinusHandler = (): void => {
        if (count + (props.value ?? 0) > 0) {
            setCount(count - 1);
        }
    };

    const incrementPlusHandler = (): void => {
        setCount(count + 1);
    };

    useEffect(() => {
        if (count + (props.value ?? 0) < 0) {
            setCount(-(props.value ?? 0));
        }
        props.onValueChange?.(count, props.id ?? 0);
    }, [count, props.value]);

    return (
        <div className="flex m-4 items-center h-max rounded-xl bg-[#70707016] p-3 max-w-[33vw]">
            <button onClick={resetHandler}>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-12 h-12 color-red">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
                </svg>
            </button>
            {props.image !== undefined ? (<img src={props.image} alt="beer" className="w-12 h-12" />) : null}
            <h1 className='grow text-3xl ml-5'>{props.name}</h1>
            {props.price !== 0 ? <h1 className='mr-6 text-2xl'>{props.price}€</h1> : null}
            <div className='flex pl-4 pr-4 border-2 rounded-full border-green-800'>
                <button onClick={incrementMinusHandler}>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1} stroke="currentColor" className="w-10 h-10">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M15 12H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                </button>
                <h3 className='p-3 text-3xl'>{count + (props.value ?? 0)}</h3>
                <button onClick={incrementPlusHandler}>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1} stroke="currentColor" className="w-10 h-10">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v6m3-3H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                </button>
            </div>
        </div>
    );
};
